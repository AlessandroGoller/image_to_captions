...
def create_inline_keyboard(list_command: list[dict]) -> types.InlineKeyboardMarkup:
    ...
    for cmd in list_command:
        button = types.InlineKeyboardButton(
            text=cmd["label"], callback_data=cmd["command"]
        )
...
async def handle_callback_query(query: types.CallbackQuery) -> str:  # noqa
    ...
    if button_data == "/help_inline":
        ...
        await bot.edit_message_text(
            "Se tutto andasse ora potresti caricare una immagine",
            chat_id=query.message.chat.id,
            message_id=query.message.message_id,
            reply_markup=None,
        )
        ...
        item["command"] for item in list_commands_after_selected_prompt
    ]:
        try:
...
async def start(message: types.Message) -> str:
    ...
    if len(hash_code) <= 1:
        await message.answer(
            f"Error hash code, {message.from_user.full_name}\n{message.text=}\n\
                        {message.chat.id=}"
        )
        return "ok"
    ...
    await message.answer(
        f"Telegram row created:, {telegram.id_user}\n{telegram.id_chat=}\n\
...
async def main_handler(message: types.Message) -> str:
    ...
    try:
        ...
        logger.info(
            f"Main: {user_id} {user_full_name} {time.asctime()}. Message: {message}"
        )
        ...
        logger.info(
            f"Main: {user_id} {user_full_name} {time.asctime()}.\
...
async def image_handler(message: types.Message) -> str:
    ...
    try:
        ...
        update_message_description(
            id_chat=message.chat.id, description=description_image
        )
        ...
        if company is None:
            ...
            return "No company"
        sample_posts = get_last_n_instagram(company_id=company.id_company, number_ig=20)
...
async def profile_settings(message: types.Message) -> None:
    ...
    try:
        ...
        await message.reply(
            f"DOVRESTI ESSERE in Settings!! =\n{profile_settings_text}",
            reply_markup=None,
        )

    except Exception as error:
        logger.error(f"ERROR during profile settings {error}\n{traceback.format_exc()}")
        await message.reply(
            f"There was an error: {error}\n-----\n{traceback.format_exc()}"
        )
