...
def add_info_sponsor(session_state: dict) -> str:
    ...
    if "option_event" in session_state:
        ...
        text = ""
        text += (
            f"""The event will be locate in {event_option[0]}. """
            if event_option[0] != ""
            else ""
        )
        text += (
            f"""The event will be in date: {event_option[1]}. """
            if event_option[1] != ""
            else ""
        )
        text += (
            f"""There more info about the event: {event_option[2]}. """
            if event_option[2] != ""
            else ""
        )
        logger.info(text)
...
def update_post(option: str) -> None:
    ...
    st.session_state["messages"].append(
        {"role": "assistant", "content": st.session_state["post"]}
    )
