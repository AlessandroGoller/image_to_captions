...
from app.utils.streamlit_utils.action_helper import (
    add_info_sponsor,
    create_new_post,
    update_post,
)
...
if company is None:
    ...
    if not st.session_state.get("image_description", False) and st.session_state.get(
        "image_cache", False
    ):
    ...
    if st.session_state.get("image_description", False):
        ...
        selected_option = st.selectbox(
            "Do you want to sponsor something? Choose an option:",
            options,
            index=index_option,
        )
        ...
        if selected_option == "Product":
            description = st.text_input(
                "Enter a brief description:", st.session_state.get("option_product", "")
            )
            ...
            if "option_event" in st.session_state:
                location = st.text_input(
                    "Enter the event location:", st.session_state.get("option_event")[0]
                )
                date = st.date_input(
                    "Enter the event date:", st.session_state.get("option_event")[1]
                )
                additional_info = st.text_area(
                    "Enter additional information:",
                    st.session_state.get("option_event")[2],
                )
            else:
        ...
        if (
            st.button("Genera il post!")
            and not st.session_state.get("post", False)
            and st.session_state.get("image_description", False)
        ):
            ...
            if sample_posts is None or len(sample_posts) < 1:
                with st.spinner("Preparation, please wait"):
                    ...
                    if instagram_account is None or instagram_account == "":
                        raise ValueError(
                            "No instagram account inserted, please insert it"
                        )
                    data = client.get_post_info_json(
                        instagram_account, last_n_posts=LAST_N_POST
                    )
                ...
                for single_post in stqdm(data, desc="Scraping Instagram"):
                    ...
                    if create_instagram(instagram=instagram_post):
                        post_inserted += 1
                logger.info(
                    f"Inserted {post_inserted=} on account ig: {company.url_instagram}"
                )
            ...
            with st.spinner("Sto generando tre post da cui potrai scegliere.."):
                ...
                prompt = create_prompt(
                    sample_posts, st.session_state["image_description"]
                )
